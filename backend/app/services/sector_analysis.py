...
async def get_portfolio_dashboard(db: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        sector_expr = func.coalesce(ApplicantProfile.employer_sector, literal("MISSING"))
        base = (
        select(
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="get_portfolio_dashboard")
        raise
...
async def get_sector_detail(db: AsyncSession, sector_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        .where(
            ...
            func.coalesce(ApplicantProfile.employer_sector, literal("MISSING")) == sector_name,
        )
        )
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="get_sector_detail")
        raise
...
async def get_sector_heatmap(db: AsyncSession) -> list[dict]:
    ...
    try:
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="get_sector_heatmap")
        raise
...
async def run_stress_test(
    ...
    try:
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="run_stress_test")
        raise
    ...
    try:
        ...
        if policy.exposure_cap_pct:
            ...
            sector_q = await db.execute(
                ...
                .where(
                    LoanApplication.status.in_(ACTIVE_STATUSES),
                    func.coalesce(ApplicantProfile.employer_sector, literal("MISSING")) == sector,
                )
            )
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="check_sector_origination")
        raise
...
async def generate_monthly_snapshot(db: AsyncSession, snapshot_date: date | None = None) -> int:
    ...
    try:
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="generate_monthly_snapshot")
        raise
...
async def evaluate_alert_rules(db: AsyncSession) -> list[SectorAlert]:
    ...
    try:
        ...
        await log_error(e, db=db, module="services.sector_analysis", function_name="evaluate_alert_rules")
        raise

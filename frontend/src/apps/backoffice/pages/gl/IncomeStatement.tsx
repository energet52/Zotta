        ...
        <>
          {items.map((row) => (
            ...
            >
              ...
              <span className="font-mono text-sm text-right tabular-nums shrink-0 ml-4">
                {fmt(row.balance)}
              </span>
            </div>
          ))}
...
export default function IncomeStatement() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const { data: res } = await api.get<IncomeStatementResponse>('/gl/income-statement', { params });
      setData(res);
    } catch {
  ...
  const netIncomePositive = data ? data.net_income >= 0 : true;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--color-text)]">Income Statement</h1>
      ...
      {loading ? (
        <Card>
          <div className="flex items-center justify-center py-20 text-[var(--color-text-muted)]">
            <Loader2 size={24} className="animate-spin mr-3" /> Loading income statement…
          </div>
        </Card>
      ) : error ? (
        <Card>
        ...
        <Card padding="none">
          <div className="divide-y divide-[var(--color-border)]">
            <SectionBlock
              title="Revenue"
              items={data.sections?.revenue?.items ?? []}
              total={data.revenue_total}
              expanded={expanded.revenue ?? true}
              onToggle={() => toggleSection('revenue')}
            />
            <SectionBlock
              title="Expenses"
              items={data.sections?.expense?.items ?? []}
              total={data.expense_total}
              expanded={expanded.expense ?? true}
          ...
          {/* Net Income */}
          <div
            className={`border-t-2 border-[var(--color-border)] px-4 py-4 font-semibold ${
              netIncomePositive ? 'bg-[var(--color-success)]/10 text-[var(--color-success)]' : 'bg-[var(--color-danger)]/10 text-[var(--color-danger)]'
            }`}
          >
            <div className="flex justify-between items-center">
              <span>Net Income</span>
              <span className="font-mono tabular-nums">
                {fmtSigned(data.net_income)}
              </span>
            </div>
          </div>
        </Card>
